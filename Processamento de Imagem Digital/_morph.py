...
class mm(object):
  ...
  @staticmethod
  def ero(f,Bc=np.ones((3,3),dtype= 'uint8')):
  ...
  @staticmethod
  def dil(f,Bc=np.ones((3,3),dtype= 'uint8')):
  ...
  @staticmethod
  def ero1(f,b=np.ones((3,3),dtype= 'uint8')):
  ...
  def dil1(f,b=np.ones((3,3),dtype= 'uint8')):
  ...
  @staticmethod
  def gradm(f,b=np.ones((3,3),dtype= 'uint8')):
  ...
  @staticmethod
  def infrec(f,g,b=np.ones((3,3),dtype= 'uint8')):
  ...
  @staticmethod
  def suprec(f,g,b=np.ones((3,3),dtype= 'uint8')):
  ...
  @staticmethod
  def closerec(f,b=np.ones((3,3),dtype= 'uint8'),bc=np.ones((3,3),dtype= 'uint8')):
    """This function will be erode g with maximum f, until converge
    input:
      - f: input image
      - b: mark image
  ...
  @staticmethod
  def asf(f,filter='OC',b=np.ones((3,3),dtype= 'uint8'),n=1):
  ...
  @staticmethod
  def openth(f,b=np.ones((3,3),dtype= 'uint8')):
      return mm.subm(f,cv2.morphologyEx(f, cv2.MORPH_OPEN, b))

  @staticmethod
  def openth1(f,b=np.ones((3,3),dtype= 'uint8')):
    return mm.subm(f, mm.dil1(mm.ero1(f,b),b) )

  @staticmethod
  def closeth(f,b=np.ones((3,3),dtype= 'uint8')):
      return mm.subm(cv2.morphologyEx(f, cv2.MORPH_CLOSE, b),f)
       
  @staticmethod
  def closerecth(f,b=np.ones((3,3),dtype= 'uint8')):
      return mm.subm(cv2.morphologyEx(f, cv2.MORPH_CLOSE, b),f)

  @staticmethod
  def open(f,b=np.ones((3,3),dtype= 'uint8')):
      return cv2.morphologyEx(f, cv2.MORPH_OPEN, b)
  
  @staticmethod
  def close(f,b=np.ones((3,3),dtype= 'uint8')):
      return cv2.morphologyEx(f, cv2.MORPH_CLOSE, b)
   
  @staticmethod
  def water0(f,b=np.ones((3,3),dtype= 'uint8'), op='region'):
  ...
  @staticmethod
  def waterB(f,m,b=np.ones((3,3),dtype= 'uint8'), op='region'):
  ...
  def watershed(f,mark,op='region'):
      ...
      if len(f): # condictional watershed by cv2
        ...
        if op=='line':
          return np.array((w-cv2.erode(w.astype('uint16'),np.ones((3,3),dtype= 'uint8')))>0).astype('uint16')
        else:
          return w
  ...
  def regmax(f,b=np.ones((3,3),dtype='uint8')):
    ...
    return mm.union(mm.threshad(g,0),mm.threshad(f,k))
  ...
  def regmin(f,b=np.ones((3,3),dtype='uint8')):
    ...
    return mm.union(mm.threshad(g,1),mm.threshad(f,0))
...
Implementei duas versões no final do arquivo _morph.py
...
Neste arquivo _morph.py estão quase todas as implementações necessárias para fazer as listas, basta saber utiliza corretamente os métodos de entrada e de processamento.
